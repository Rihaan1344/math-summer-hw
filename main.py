...
def investigate(place1_index: int, place2_index: int, inv_num: int) -> None:
    ...
    st.write(f"Accounting for the curvature, an online tool tells us that the distance is {
...
if can_calculate:
    ...
    # estimate_point_1 = (data.loc[0, "lon"], data.loc[0, "lat"])
    # estimate_point_2 = (data.loc[1, "lon"], data.loc[1, "lat"])

    # real_point_1 = (data.loc[0, "lat"], data.loc[0, "lon"])
    # real_point_2 = (data.loc[1, "lat"], data.loc[1, "lon"])

    # st.divider()
    # st.subheader(f"_Investigation 1:_ {place1} and {place2}")
    # st.write("When we use the latitudes and longitudes of these places as x and y values, we can use the distance formula,")
    # st.latex("d = \sqrt{(x_{2} - x_{1})^2 + (y_{2} - y_{1})^2},")
    # st.write("then we can calculate the distance between the places!")
    # st.write(
